...
* Uses MySQLi
* Server-side logging through the javascript console object and a the database
* Messages can be sent to the browser via \Walleye\Console::alert() and gritter
* Secure session management
* Includes basic sql
* Uses the Walleye namespace
* Includes phpDoc
* Unit tests can be found in the tests folder

== How to use the framework

Set up the configuration options in walleye.config.php.
...
Now you have all the functions available to you through MySQLi. You can change the database that you are connecting to by passing the name as a parameter.
	...
	$db = new \Walleye\Database('test_db');
